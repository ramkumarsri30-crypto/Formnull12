...
snap
$S find text "Create workspace" click 2>&1 | tee -a "$LOG"
$S wait 800 >/dev/null 2>&1
...
$S find role button click --name "Switch workspace" 2>&1 | tee -a "$LOG"
$S wait 800 >/dev/null 2>&1
$S find text "formnull.test's workspace" click 2>&1 | tee -a "$LOG"
$S wait --load networkidle >/dev/null 2>&1
$S wait 2000 >/dev/null 2>&1
...
$S find role button click --name "Switch workspace" 2>&1 | tee -a "$LOG"
$S wait 800 >/dev/null 2>&1
$S find text "Phase 2A Final Workspace Test" click 2>&1 | tee -a "$LOG"
$S wait --load networkidle >/dev/null 2>&1
$S wait 2000 >/dev/null 2>&1
